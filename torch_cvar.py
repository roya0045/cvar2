...
class TvarLayer(nn.Module):  # K.layers.convolutional._Conv):#layers.Layer):
    ...
    def __init__(self, num_c, filtr,inputshape, stride=1,  # num_routing=3,
    ...
    def build(self, input_shape, dtype=torch.double):
        ...
        if self.format == 'NHWC':
            ...
            if self.IDC:
                if self.SCH:
                    w = torch.empty(self.noutputs, self.window[0] * self.window[1] * self.ch, 1, dtype=dtype)
                else:
                    w = torch.empty(self.noutputs, self.window[0] * self.window[1], self.ch, 1, dtype=dtype)
            else:
                w = torch.empty(self.noutputs, self.window[0] * self.window[1], 1, dtype=dtype)
            finit.kaiming_normal_(w)
            ...
            if self.IDC:
                if self.SCH:
                    w = torch.empty(self.noutputs, self.ch * self.window[0] * self.window[1], 1, dtype=dtype)
                else:
                    w = torch.empty(self.noutputs, self.ch, self.window[0] * self.window[1], 1, dtype=dtype)
            else:
                w = torch.empty(self.noutputs, self.window[0] * self.window[1], 1, dtype=dtype)
        ...
        if self.SCH:
            b = torch.empty(self.noutputs * self.ch, 1, 1, dtype=dtype)
        elif self.IDC:
            b = torch.empty(self.noutputs, self.ch, 1, 1, dtype=dtype)
        else:
            b = torch.empty(self.noutputs, 1, 1, dtype=dtype)
